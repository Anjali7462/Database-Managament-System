...
db = mysql.connector.connect(
    host="localhost", user="root", password="", database="inventory"
)
...
def main_menu():
    ...
    new_order_button = Button(
        menu_window, text="New Order", command=open_new_order_page
    )
    update_products_button = Button(
        menu_window, text="Update Products", command=open_update_products_page
    )
...
def new_order_page():
    ...
    description_label = Label(order_window, text="Description")
    description_label.grid(row=1, column=0)
    description_entry = Entry(order_window)
    description_entry.grid(row=1, column=1)

    category_id_label = Label(order_window, text="Category ID")
    category_id_label.grid(row=2, column=0)
    category_id_entry = Entry(order_window)
    category_id_entry.grid(row=2, column=1)

    price_label = Label(order_window, text="Price")
    price_label.grid(row=3, column=0)
    price_entry = Entry(order_window)
    price_entry.grid(row=3, column=1)

    stock_quantity_label = Label(order_window, text="Stock Quantity")
    stock_quantity_label.grid(row=4, column=0)
    stock_quantity_entry = Entry(order_window)
    stock_quantity_entry.grid(row=4, column=1)

    add_button = Button(order_window, text="Add Product", command=add_product)
    ...
    order_tree.heading("CategoryID", text="Category ID")
    order_tree.heading("Price", text="Price")
    order_tree.heading("StockQuantity", text="Stock Quantity")

    order_tree.column("#1", width=100, anchor="center")
    order_tree.column("#2", width=210, anchor="center")
    order_tree.column("#3", width=270, anchor="center")
    order_tree.column("#4", width=175, anchor="center")
    order_tree.column("#5", width=120, anchor="center")
    order_tree.column("#6", width=150, anchor="center")

    order_tree["show"] = "headings"
    order_tree.grid(row=6, column=0, columnspan=6)
...
def new_order_page():
    ...
    def add_to_cart():
        ...
        if product:
            ...
            order_tree.insert(
                "", "end", values=(product_id, product_name, quantity, price, total)
            )
        else:
    ...
    order_tree = ttk.Treeview(
        order_window,
        columns=("ProductID", "ProductName", "Quantity", "Price", "Total"),
    )
    order_tree.heading("ProductID", text="Product ID")
    order_tree.heading("ProductName", text="Product Name")
...
def update_products_page():
    ...
    new_stock_quantity_label = Label(
        update_window, text="New Stock Quantity (for update)"
    )
    ...
    update_button = Button(
        update_window, text="Update Stock Quantity", command=update_product
    )
